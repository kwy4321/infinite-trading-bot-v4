...
class GoogleSheetsLedger:
    ...
    def _client(self):
        ...
        path = Path(self.settings.google_service_account_json)
        if not path.is_file():
    ...
    def _rows_from_dicts(items: list[dict], headers: list[str]) -> list[list]:
        return [headers] + [[item.get(h, "") for h in headers] for item in items]
    ...
    def _write_tab(spreadsheet, title: str, rows: list[list]) -> None:
        try:
            ...
            ws = spreadsheet.add_worksheet(title=title, rows=max(len(rows), 2), cols=max(len(rows[0]) if rows else 1, 1))
        ws.clear()
        if rows:
    ...
    def sync_all(self) -> dict:
        ...
        snapshot = collect_portfolio_snapshot(self.app)
        ...
        self._write_tab(spreadsheet, "Dashboard", self._rows_from_dicts(dashboard_row, DASHBOARD_HEADERS))
        self._write_tab(spreadsheet, "Status", self._rows_from_dicts(snapshot["symbols"], STATUS_HEADERS))
        self._write_tab(spreadsheet, "Trades", self._rows_from_dicts(trades, TRADES_HEADERS))
        self._write_tab(spreadsheet, "Cycles", self._rows_from_dicts(cycles, CYCLES_HEADERS))
        self._write_tab(spreadsheet, "Monthly", self._rows_from_dicts(monthly, MONTHLY_HEADERS))

        msg = (
            f"Sheets 동기화 완료 — 매매 {len(trades)}건 · 완료회차 {len(cycles)}건"
        )
        logger.info(msg)
        return {
            "ok": True,
