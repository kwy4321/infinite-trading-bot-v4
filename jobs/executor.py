...
from tg.notifications import (
    format_market_open,
    format_market_open_report,
    format_market_open_start,
    format_order_filled,
...
class JobExecutor:
    ...
    async def _already_traded_for_us_session(
        ...
        """해당 KST일 18:05 이전 LOC 접수가 있으면 True."""
        if st is None:
            st = self.app.state.load(symbol)
        submit_kst = await self._loc_auto_submit_kst(us_date)
        if has_us_session_fill_in_state(
            st, symbol, us_date, self.app.cycles, submit_kst,
        ):
            return True
        if not self._is_dry():
            try:
                return await asyncio.to_thread(
                    has_us_session_fill_from_broker,
                    self.app.broker, symbol, us_date, submit_kst,
                )
            except Exception:
    ...
    async def run_phase(
        ...
        if phase != JobPhase.JOB4_REPORT:
            ...
            try:
                ...
                return
            label = self._phase_label(phase, submit_at_open=submit_at_open)
            if not cal_ok:
                await self._notify(
        ...
        if is_loc:
            ...
            if submit_at_open:
                await self._notify(
                    format_market_open_start(now, len(symbols)), html=True,
                )
            else:
                await self._notify(
                    f"🔔 <b>LOC 실행</b>  <i>{now} KST</i>\n"
                    f"종목 {len(symbols)}개",
                    html=True,
                )
        ...
        if is_loc:
            ...
            if submit_at_open:
                await self._notify(
                    format_market_open_report(
                        now, lines, total_sub, total_orders,
                    ),
                    html=True,
                )
            else:
    ...
    async def run_morning_briefing(self, *, scheduled: bool = True) -> None:
        if scheduled:
            ...
            if should_skip_scheduled_briefing():
                logger.info("morning briefing skipped — US market closed (Sun/Mon KST)")
                return
        try:
    ...
    async def run_job3(self, premium: int | None = None, *, scheduled: bool = True, **_):
        ...
        if scheduled and not self.app.runtime.is_paused():
            ...
            if symbols and len(skipped) == len(symbols):
                ...
                await self._notify(
                    f"⏭️ <b>LOC 접수 스킵</b>\n"
                    f"📅 <b>{target}</b> — 18:05 이전 LOC 이미 접수됨\n"
                ...
                return
        await self.run_phase(
            JobPhase.JOB3_LOC_CLOSE, premium, submit_at_open=True, force=not scheduled,
        )
    ...
    async def force_job(self, name: str, premium: int | None = None) -> None:
        ...
        mapping = {
            ...
            "job4": self.run_job4,
            "briefing": lambda **kw: self.run_morning_briefing(scheduled=False),
        }
