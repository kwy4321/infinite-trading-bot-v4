...
class InfiniteStrategyV40:
    ...
    def calc_one_buy_amount(self, cash: float, t_val: float, split_count: int) -> float:
    ...
    def get_plan(
        ...
        cash: float, split_count: int, principal: float,
    ) -> dict:
        ...
        plan = {
            "mode": mode.value, "star_pct": round(star_pct, 4), "star_price": 0.0,
            "one_buy_amount": 0.0, "buy_orders": [], "sell_orders": [],
        }

        if mode == TradingMode.ENTRY:
            one_buy = self.calc_one_buy_amount(cash or principal, 0, split_count)
            plan["one_buy_amount"] = round(one_buy, 2)
        ...
        if mode == TradingMode.REVERSE:
            one_buy = self.calc_one_buy_amount(cash, t_val, split_count)
            plan["one_buy_amount"] = round(one_buy, 2)
            star_price = self.calc_star_price(avg_price, star_pct)
        ...
        star_price = self.calc_star_price(avg_price, star_pct)
        plan["star_price"] = star_price
        star_buy = self.calc_buy_trigger_price(star_price)
    ...
    def summarize(self, ticker, current_price, avg_price, qty, t_val, cash, split_count):
        ...
        one_buy = self.calc_one_buy_amount(cash, t_val, split_count) if qty > 0 else 0.0
        return {
