...
def format_home(app: App, token_line: str) -> str:
    ...
    lines = [
        ...
        quote(
            ...
            row("📊", "장부", dim("버튼 → Streamlit · Google Sheets")),
            row("⚙️", "설정", dim("원금 · 분할 · 리버스 · 거래 종목")),
            row("🔑", "토큰", dim("만료 시각 · 갱신")),
        ),
        "",
        subsection("추가 명령"),
        quote(
            dim("/dashboard  장부 (Streamlit · Sheets 링크)"),
            dim("/sheets_sync  Google Sheets 수동 동기화"),
            dim("/sync  실계좌 T·체결 동기화"),
